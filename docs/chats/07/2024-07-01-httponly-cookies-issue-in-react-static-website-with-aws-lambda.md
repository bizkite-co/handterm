---
original file name: Codeium Chat - e5zJmYRf2TYH3KN9By6HJjzDwpBKsFVG.md
summary: The user is facing an issue with HttpOnly Cookies not being sent in requests to their server-side code in a React static website using AWS Lambda functions. The user shared their code snippets, showing the client-side setup using Axios with `withCredentials: true` and the server-side Lambda function `getUser` that is supposed to parse the cookies. Codeium provided recommendations, including checking CORS server-side configurations, ensuring correct domain and path for cookies, and adjusting cookie attributes like Secure and SameSite.

The user further shared their server-side Lambda functions for signin, signup, and changePassword, which had CORS configuration issues with an overly permissive `Access-Control-Allow-Origin` header value, potentially causing the request to fail to include cookies. Codeium suggested dynamically setting `Access-Control-Allow-Origin` based on the request's `Origin` header in each Lambda function as a solution.

In the continuation of the conversation, Codeium proposed centralizing CORS configuration using AWS CDK's HTTP API with CORS settings, which handles CORS policies at the API Gateway level. Codeium provided a CDK stack example demonstrating how to create an HTTP API with CORS configuration and Lambda integrations. This approach removes the need to manually set CORS headers in each Lambda function and simplifies CORS policy management. The user inquired about removing CORS headers from individual Lambdas, to which Codeium confirmed that API Gateway's CORS settings would handle CORS headers, eliminating the need for manual configuration in Lambdas.
date: 2024-07-01T19:38
---
